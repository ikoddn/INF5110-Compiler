...
public class CodeProcedure {
		...
		for (int i = 0; i < this.instructions.size(); i++) {
			...
			if (instruction instanceof JMP) {
				newInstructions.add(i,
						new JMP(findSize(((JMP) instruction).getJumpTo())));
			} else if (instruction instanceof JMPFALSE) {
				newInstructions.add(i, new JMPFALSE(
						findSize(((JMPFALSE) instruction).getJumpTo())));
			} else if (instruction instanceof JMPTRUE) {
				newInstructions.add(i, new JMPTRUE(
						findSize(((JMPTRUE) instruction).getJumpTo())));
			} else {
	...
	public byte[] getBytecode() {
		...
		NumberConversion.shortToByteArray(bytes, 2,
				(short) this.parameterTypes.size());
		NumberConversion.shortToByteArray(bytes, 4,
				(short) this.variableTypes.size());
		NumberConversion.shortToByteArray(bytes, 6,
				(short) this.instructions.size());
