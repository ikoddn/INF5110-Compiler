...
public class CodeFile {
	...
	public byte[] getBytecode() {
		...
		for (int i = 0; i < this.variableTypes.size(); i++) {
			variableTypesBytes[i] = this.variableTypes.get(new Integer(i))
					.getBytecode();
			totalSize += variableTypesBytes[i].length;
		}
		byte[][] proceduresBytes = new byte[this.procedures.size()][];
		for (int i = 0; i < this.procedures.size(); i++) {
			proceduresBytes[i] = this.procedures.get(new Integer(i))
					.getBytecode();
		...
		NumberConversion.shortToByteArray(bytes, 2,
				(short) this.variableNames.size());
		NumberConversion.shortToByteArray(bytes, 4,
				(short) this.procedures.size());
		NumberConversion
				.shortToByteArray(bytes, 6, (short) this.structs.size());
		NumberConversion.shortToByteArray(bytes, 8,
				(short) this.stringConstants.size());

		int index = 10;
		// Variables
		// First the sizes
		for (int i = 0; i < variableNamesBytes.length; i++) {
			NumberConversion.shortToByteArray(bytes, index,
					(short) variableNamesBytes[i].length);
			index += 2;
		}
		// Then the values
		for (int i = 0; i < variableNamesBytes.length; i++) {
		...
		for (int i = 0; i < proceduresBytes.length; i++) {
			NumberConversion.shortToByteArray(bytes, index,
					(short) proceduresBytes[i].length);
			index += 2;
		}
		// Then the values
		for (int i = 0; i < proceduresBytes.length; i++) {
		...
		for (int i = 0; i < stringConstantsBytes.length; i++) {
			NumberConversion.shortToByteArray(bytes, index,
					(short) stringConstantsBytes[i].length);
			index += 2;
		}
		// Then the values
		for (int i = 0; i < stringConstantsBytes.length; i++) {
