...
public class Interpreter {
	...
	public void run() {
		...
		ActivationBlock current = new ActivationBlock(
				this.procedures[this.mainNum]);
		...
		while (true) {
			...
			case 3:
				// CALL
				if (this.procedures[NumberConversion.shortFromByteArray(
						current.procedure.bytecode, pc + 1)].bytecode.length == 0) {
					invokeLibProcedure(
							this.procedures[NumberConversion.shortFromByteArray(
									current.procedure.bytecode, pc + 1)], stack);
					pc += 3;
				} else {
					current.pc = pc + 3;
					ActivationBlock callBlock = new ActivationBlock(
							this.procedures[NumberConversion
									.shortFromByteArray(
			...
			case 6:
				// GETFIELD
				stack.push(getfield(heap, NumberConversion.shortFromByteArray(
						current.procedure.bytecode, pc + 1),
						this.structs[NumberConversion.shortFromByteArray(
								current.procedure.bytecode, pc + 3)], stack
								.pop()));
				pc += 5;
				break;
			case 7:
			...
			case 8:
				// JMP
				pc = NumberConversion.shortFromByteArray(
						current.procedure.bytecode, pc + 1);
				break;
			case 9:
				// JMPFALSE
			...
			case 11:
				// LOADGLOBAL
				stack.push(globals[NumberConversion.shortFromByteArray(
						current.procedure.bytecode, pc + 1)]);
				pc += 3;
				break;
			case 12:
				// LOADLOCAL
				stack.push(current.variables[NumberConversion
						.shortFromByteArray(current.procedure.bytecode, pc + 1)]);
				pc += 3;
				break;
			case 13:
				// LOADOUTER
			...
			case 19:
				// PUSHFLOAT
				float floatValue = NumberConversion.floatFromByteArray(
						current.procedure.bytecode, pc + 1);
			...
			case 20:
				// PUSHINT
				int intValue = NumberConversion.intFromByteArray(
						current.procedure.bytecode, pc + 1);
			...
			case 22:
				// PUSHSTRING
				String stringValue = this.constants[NumberConversion
						.shortFromByteArray(current.procedure.bytecode, pc + 1)];
			...
			case 23:
				// PUTFIELD
				putfield(heap, NumberConversion.shortFromByteArray(
						current.procedure.bytecode, pc + 1),
						this.structs[NumberConversion.shortFromByteArray(
								current.procedure.bytecode, pc + 3)],
						stack.pop(), stack.pop());
				pc += 5;
				break;
			case 24:
				...
				if (current.procedure == this.procedures[this.mainNum])
					return;
				...
				if (current.procedure.returnType instanceof VoidType) {
					...
					if (retvalue instanceof Integer
							&& current.procedure.returnType == FloatType.TYPE) {
			...
			case 25:
				...
				if (this.variables[NumberConversion.shortFromByteArray(
						current.procedure.bytecode, pc + 1)].type instanceof FloatType
						&& globalValue instanceof Integer) {
					globalValue = new Float(((Integer) globalValue).intValue());
				}
				globals[NumberConversion.shortFromByteArray(
						current.procedure.bytecode, pc + 1)] = globalValue;
				pc += 3;
				break;
			case 26:
				// STORELOCAL
				Object localValue = stack.pop();
				if (current.variables[NumberConversion.shortFromByteArray(
						current.procedure.bytecode, pc + 1)] instanceof Float
						&& localValue instanceof Integer) {
					localValue = new Float(((Integer) localValue).intValue());
				}
				current.variables[NumberConversion.shortFromByteArray(
						current.procedure.bytecode, pc + 1)] = localValue;
				pc += 3;
				break;
			case 27:
				// STOREOUTER
			...
			default:
				System.out.println("UNKNOWN INSTRUCTION: "
						+ current.procedure.bytecode[pc] + " at " + pc + " in "
	...
	private void initializeParams(ActivationBlock current, Stack stack) {
		...
		for (int i = parSize - 1; i >= 0; i--) {
			...
			if (current.procedure.parameters[i] instanceof FloatType
					&& current.variables[i] instanceof Integer) {
				current.variables[i] = new Float(
						((Integer) current.variables[i]).intValue());
			}
		}
	}
	...
	private void initializeCallVars(ActivationBlock current) {
		...
		for (int i = 0; i < varSize; i++) {
			current.variables[parSize + i] = current.procedure.variables[i]
					.initialValue();
		}
	}
			...
			if (procedure.name.equals("readint")) {
				...
				if (line.length() != 1) {
					throw new RuntimeException(
							"readchar can only read line of length 1");
				}
				...
				throw new RuntimeException("Could not find library procedure "
						+ procedure.name);
			}
		} catch (IOException e) {
			throw new RuntimeException("Could not invoke library procedure "
					+ procedure.name + "because of error " + e);
		}
	}

	private Object add(Object pop0, Object pop1) {
		if (pop1 instanceof Integer && pop0 instanceof Integer) {
			return new Integer(((Integer) pop1).intValue()
					+ ((Integer) pop0).intValue());
		} else if (pop1 instanceof Integer && pop0 instanceof Float) {
			return new Float(((Integer) pop1).floatValue()
					+ ((Float) pop0).floatValue());
		} else if (pop1 instanceof Float && pop0 instanceof Integer) {
			return new Float(((Float) pop1).floatValue()
					+ ((Integer) pop0).floatValue());
			...
			throw new RuntimeException("ADD not possible for " + pop1 + " and "
					+ pop0);
		}
	}
	...
	private Object sub(Object pop0, Object pop1) {
		if (pop1 instanceof Integer && pop0 instanceof Integer) {
			...
			throw new RuntimeException("SUB not possible for " + pop1 + " and "
					+ pop0);
		}
	}
	...
	private Object mul(Object pop0, Object pop1) {
		if (pop1 instanceof Integer && pop0 instanceof Integer) {
			...
			throw new RuntimeException("MUL not possible for " + pop1 + " and "
					+ pop0);
		}
	}
	...
	private Object and(Object pop0, Object pop1) {
		if (pop1 instanceof Boolean && pop0 instanceof Boolean) {
			...
			throw new RuntimeException("AND not possible for " + pop1 + " and "
					+ pop0);
		}
	}
	...
	private Object eq(Object pop0, Object pop1) {
		if (pop1 instanceof Integer && pop0 instanceof Integer) {
			...
			throw new RuntimeException("EQ not possible for " + pop1 + " and "
					+ pop0);
		}
	}
	...
	private Object exp(Object pop0, Object pop1) {
		if (pop1 instanceof Integer && pop0 instanceof Integer) {
			return new Float(Math.pow(((Integer) pop1).doubleValue(),
					((Integer) pop0).doubleValue()));
		} else if (pop1 instanceof Float && pop0 instanceof Integer) {
			return new Float(Math.pow(((Float) pop1).doubleValue(),
					((Integer) pop0).doubleValue()));
		} else {
			throw new RuntimeException("EXP not possible for " + pop1 + " and "
					+ pop0);
		}
	}
	...
	private Object gt(Object pop0, Object pop1) {
		if (pop1 instanceof Integer && pop0 instanceof Integer) {
			return new Boolean(
					((Integer) pop1).intValue() > ((Integer) pop0).intValue());
		} else if (pop1 instanceof Integer && pop0 instanceof Float) {
			return new Boolean(
					((Integer) pop1).floatValue() > ((Float) pop0).floatValue());
		} else if (pop1 instanceof Float && pop0 instanceof Integer) {
			return new Boolean(
					((Float) pop1).floatValue() > ((Integer) pop0).floatValue());
		} else if (pop1 instanceof Float && pop0 instanceof Float) {
			return new Boolean(
					((Float) pop1).floatValue() > ((Float) pop0).floatValue());
		} else {
			throw new RuntimeException("GT not possible for " + pop1 + " and "
					+ pop0);
		}
	}
	...
	private Object gteq(Object pop0, Object pop1) {
		if (pop1 instanceof Integer && pop0 instanceof Integer) {
			...
			return new Boolean(
					((Integer) pop1).floatValue() >= ((Float) pop0)
							.floatValue());
		} else if (pop1 instanceof Float && pop0 instanceof Integer) {
			return new Boolean(
					((Float) pop1).floatValue() >= ((Integer) pop0)
							.floatValue());
		} else if (pop1 instanceof Float && pop0 instanceof Float) {
			return new Boolean(
					((Float) pop1).floatValue() >= ((Float) pop0).floatValue());
		} else {
			throw new RuntimeException("GTEQ not possible for " + pop1
					+ " and " + pop0);
		}
	}
	...
	private Object lt(Object pop0, Object pop1) {
		if (pop1 instanceof Integer && pop0 instanceof Integer) {
			...
			throw new RuntimeException("LT not possible for " + pop1 + " and "
					+ pop0);
		}
	}
	...
	private Object lteq(Object pop0, Object pop1) {
		if (pop1 instanceof Integer && pop0 instanceof Integer) {
			...
			throw new RuntimeException("LTEQ not possible for " + pop1
					+ " and " + pop0);
		}
	}
	...
	private Object neq(Object pop0, Object pop1) {
		if (pop1 instanceof Integer && pop0 instanceof Integer) {
			...
			throw new RuntimeException("NEQ not possible for " + pop1 + " and "
					+ pop0);
		}
	}
	...
	private Object or(Object pop0, Object pop1) {
		if (pop1 instanceof Boolean && pop0 instanceof Boolean) {
			return ((Boolean) pop1).booleanValue()
					|| ((Boolean) pop0).booleanValue();
		} else {
			throw new RuntimeException("OR not possible for " + pop1 + " and "
					+ pop0);
		}
	}
	...
	private void putfield(Heap heap, short fieldNumber, Struct struct,
			Object pop0, Object pop1) {
	...
	private Object getfield(Heap heap, short fieldNumber, Struct struct,
			Object pop1) {
