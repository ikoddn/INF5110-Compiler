...
public class Loader {
	...
	private CodeType codeType(byte[] bytes, int index) {
		...
		case 1:
			Short struct = NumberConversion
					.shortFromByteArray(bytes, index + 1);
	...
	private Procedure procedure(byte[] bytes, int index, int size) {
		...
		Procedure procedure = new Procedure(name, returnType, parameters,
				variables, instructions, bytecode);
		return procedure;
	}
	...
	private Instruction instruction(byte[] bytes, int index) {
		...
		case 19:
			float floatValue = NumberConversion.floatFromByteArray(bytes,
					index + 1);
