		...
		if (file.isDirectory()) {
			...
			for (int i = 0; i < files.length; i++) {
				...
				boolean shouldFail = filename
						.contains("fail".subSequence(0, 4));
				String fallPass = shouldFail ? "FAIL" : "PASS";
				System.out.println("Test[" + (i + 1) + "] SHOULD " + fallPass
						+ ": " + filename);
				Compiler compiler = new Compiler(this.path + File.separator
		...
		try {
			...
			default:
				if (shouldFail) {
					testOk = false;
					System.out.println("BAD: Test no " + number
							+ " didn't fail when it should!");
				}
			}
		} catch (Exception e) {
			System.out.println("Test aborted because of error in Compiler: "
					+ e);
			e.printStackTrace();
			System.exit(1);
		}
